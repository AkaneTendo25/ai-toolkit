    ...
    def calculate_loss(
        ...
        if self.train_config.max_loss_debug and self.train_config.max_loss is not None:
            if total_loss.item() > self.train_config.max_loss:
                print_acc(f"Loss {total_loss.item()} is greater than max loss {self.train_config.max_loss}. Clipping to max loss.")
                print_acc(f"timesteps: {timesteps}")

        if self.train_config.max_loss is not None:
            total_loss = torch.clamp(total_loss, max=self.train_config.max_loss)
